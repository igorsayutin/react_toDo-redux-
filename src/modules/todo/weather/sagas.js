...
export default function* onFetchWeather() {
  ...
  const weather_url = `http://api.openweathermap.org/data/2.5/weather?appid=3b2dce7c397645e8583f51b27d0279dc&units=metric&q=Kharkiv`;

  yield call(function* fetchWeatherByLocation() {
    try {
      const weather = yield fetch(weather_url).then(response =>
        response.json()
      );
